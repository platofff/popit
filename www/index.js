...
startButton.addEventListener('click', () => {
  ...
  ws.onopen = () => {
    const req = token === undefined ? { method: 'auth' } : { method: 'auth', token }
  ...
  ws.onmessage = (event) => {
    ...
    ws.onmessage = (event) => {
      ...
      if (game.move === game.you) { status.innerHTML = 'Ход: <span style="color:green">твой</span>' } else { status.innerHTML = 'Ход: <span style="color:red">противника</span>' }
      if (game.final !== undefined) {
        ...
        startButton.addEventListener('click', () => {
          ws.send(JSON.stringify({ method: 'get_game', token }))
        })
        return
      }
      for (const [i, pop] of document.querySelectorAll('.pop').entries()) {
        if (game.poped.includes(i)) {
          ...
          if (!playedAudio.includes(i)) { audio[Math.floor(Math.random() * audio.length)].play() }
          playedAudio.push(i)
        } else {
          pop.classList.remove('selected')
          pop.classList.add('unpressed')
    ...
    if (room.value !== '') { ws.send(JSON.stringify({ method: 'get_game', token, gameToken: room.value })) } else { ws.send(JSON.stringify({ method: 'get_game', token })) }
  }
})
...
popit.addEventListener('pointermove', (e) => {
  ...
  if (pop.classList.contains('pop') && game.move === game.you && !pop.classList.contains('selected') && !pop.classList.contains('pressed') && selectedX !== null && filledCount < 4) {
    ...
    if (selectedX === x) {
      if (![null, 'x'].includes(fillType) || Math.abs(selectedY - y) > 1) { return }
      fillType = 'x'
    } else if (selectedY === y) {
      if (![null, 'y'].includes(fillType) || Math.abs(selectedX - x) > 1) { return }
      fillType = 'y'
    } else return
    filledCount++
    selectedX = x
...
popit.addEventListener('pointerup', (e) => {
  ...
  filledCount = 0
  fillType = null
  ws.send(JSON.stringify({ method: 'update', poped, token, game: game.token }))
})
